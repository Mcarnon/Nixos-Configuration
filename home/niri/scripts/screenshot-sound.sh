...
# screenshot-sound — 截图快门音效守护（SHORiN 原版逻辑）
# binds 里按截图键会 `pkill -f -USR1 screenshot-sound` 上膛，随后 wl-paste
# 监听发现剪贴板出现图片时播放快门声。
# @SOUNDS_DIR@ 由 niri.nix 的 replaceStrings 注入为 store 路径。

# =================配置区域=================
SOUND="@SOUNDS_DIR@/stereo/camera-shutter.oga"
# 这是一个"扳机"文件，存于内存中 (/dev/shm)，读写极快
TRIGGER_FILE="/dev/shm/niri_screenshot_armed"
# 有效期：按下截图键后，多少秒内产生了图片才响？
TIMEOUT_SEC=15
# =========================================
